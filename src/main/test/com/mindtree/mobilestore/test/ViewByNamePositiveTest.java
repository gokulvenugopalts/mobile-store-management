...
public class ViewByNamePositiveTest {
	...
	public void setUp() throws Exception {
		...
		Mockito.when(viewObj.viewServiceByName(userMockObj)).thenReturn(res);
	}

	@Test
	public void test() {
		try {
			assertEquals(viewObj.viewServiceByName(userMockObj),res);
			System.out.println("Test Case: Finding records of user in database with name as search type is successful");
		} catch(AssertionError asser)
		{
			System.out.println("Test Case: Finding records of user in database with name as search type failed");
			fail();
		}
	}
